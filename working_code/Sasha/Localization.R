...
#Do a bootstrap to get error bars
env.number = apply(ppi_by_env, 1, sum, na.rm = T)
number.of.env.colcalized.in.DMSO = matrix(NA, 1000, ncol(ppi_by_env))
colnames(number.of.env.colcalized.in.DMSO) = 1:9
for(j in 1:1000){
  for(i in 1:ncol(ppi_by_env)){
    x = ppi[env.number == i ,] #PPIs seen in i env
    x = x[sample(1:nrow(x), floor(nrow(x)/2)),]
    y = find_colacalized(x)
    number.of.env.colcalized.in.DMSO[j,i] = percent_colacalized(y)
  }
}
m = apply(number.of.env.colcalized.in.DMSO, 2, mean, na.rm = T)*100
sd = apply(number.of.env.colcalized.in.DMSO, 2, sd, na.rm = T)*100
sem = sd/sqrt(1000)
u = m+sd
l = m-sd

plot(1:9, m, xlab = "Number of environments in which a PPI is observed", 
        ylab = "Percent colocalized in synthethic media", type = 'b', ylim = c(30,90))
