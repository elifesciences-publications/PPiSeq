...
csvWriter(PPI_carbon_final_order, "Working_data_2/Glucose_transporter/PPI_HXT_environments.csv")

# Take the order the same with Figure2A
PPI_carbon_final_order = dataFrameReader_T("Working_data_2/Glucose_transporter/PPI_HXT_environments.csv")
...
pdf("Working_figure/Figure5_environment_dynamics/Figure5A_carbonhydrate_transport_fitness_environment_primary_cluster.pdf", 
    width = 7, height = 6)
grid::grid.newpage()
...
PPI_transcription = check_specific_protein(PPI_fit, Gene_transcription) # 785
PPI_translation = check_specific_protein(PPI_fit, Gene_translation) #633
...
PPI_MED11 = PPI_transcription_fit[grep("YMR112C", PPI_transcription_fit[,1]),] #464
Group = rep("MED11", nrow(PPI_MED11))
PPI_MED11 = data.frame(PPI_MED11,Group )
PPI_other = PPI_transcription_fit_order[which(!PPI_transcription_fit_order[,1] %in% PPI_MED11[,1]),] # 321
Group = rep("Others", nrow(PPI_other))
PPI_other = data.frame(PPI_other, Group)
...
csvWriter(PPI_transcription_final, "Working_data_2/Glucose_transporter/PPI_transcription_environments.csv")

PPI_transcription_final = dataFrameReader_T("Working_data_2/Glucose_transporter/PPI_transcription_environments.csv")
...
save_pheatmap_pdf(fit_heatmap, "Working_figure/Figure5_environment_dynamics/Transcription_translation_not_use/Figure5_transcription_fitness_environment_primary.pdf")

##### Translation correlated PPIs
PPI_translation_fit = PPI_fit[which(as.character(PPI_fit[,1]) %in% PPI_translation),] #633
PPI_translation_order = sort(PPI_translation_fit[,1]) #633
PPI_translation_fit_order = PPI_translation_fit[match(PPI_translation_order, PPI_translation_fit[,1]),]
PPI_DED1 = PPI_translation_fit[grep("YOR204W", PPI_translation_fit[,1]),] #425
Group = rep("DED1", nrow(PPI_DED1))
PPI_DED1 = data.frame(PPI_DED1,Group )
PPI_other = PPI_translation_fit_order[which(!PPI_translation_fit_order[,1] %in% PPI_DED1[,1]),] # 208
Group = rep("Others", nrow(PPI_other))
PPI_other = data.frame(PPI_other, Group)
...
csvWriter(PPI_translation_final, "Working_data_2/Glucose_transporter/PPI_translation_environments.csv")


PPI_translation_final = dataFrameReader_T("Working_data_2/Glucose_transporter/PPI_translation_environments.csv")
...
fit_heatmap = pheatmap(PPI_translation_heatmap, cluster_rows = FALSE, cluster_cols = FALSE, show_rownames=FALSE,
                       annotation_colors = my_colour,angle_col = 90,
                       annotation_row = row_ann, show_colnames=T, col = color_scale)

save_pheatmap_pdf(fit_heatmap, "Working_figure/Figure5_environment_dynamics/Transcription_translation_not_use/Figure5_translation_fitness_environment_primary.pdf")
