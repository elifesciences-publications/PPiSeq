...
p <- ggplot(df, aes(x=env.number, y=ppi_coex, fill=env.number)) +   
  geom_boxplot(notch = T) +  labs(x="Environments in which a PPI is observed", y = "Co-expression mutual mank")
p + theme_classic() + theme(axis.text.y = element_text(face="bold",  size=10, angle=45), 
