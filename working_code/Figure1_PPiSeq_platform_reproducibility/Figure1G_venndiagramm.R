...
yeast_PPI = read.delim("Paper_data/Outside_datasets/BIOGRID-ORGANISM-3.5.165.tab2/BIOGRID-ORGANISM-Saccharomyces_cerevisiae_S288c-3.5.165.tab2.txt", header = T)
...
PPI_paper= read.delim(file = "Paper_data/Outside_datasets/PPI_set_PCA_science.txt", sep=" ")
...
library(VennDiagram)
pdf("Working_figure/Figure1/Figure1G_draft_Venn diagram of overlapped PPIs among PPiseq, PCA, BIOGRID_new.pdf", width = 5, height =5)
...
a = nrow(BIOGRID_PPI_uncover) # 94354
b = length(PCA_PPiseq_uncover) #395
c = length(intersect(BIOGRID_PPI_uncover[,1], PCA_PPiseq_uncover)) # 85
a-c # 94269
b-c # 310
nrow(all_PPI) # 1588857
...
n123 = nrow(BIOGRID_PPiseq_PCA_overlap) 
library(VennDiagram)
pdf("Working_figure/Figure1/Figure1G_draft_all_Venn diagram of overlapped PPIs among PPiseq, PCA, BIOGRID_new.pdf", width = 5, height =5)
draw.triple.venn(area1, area2, area3, n12, n23, n13, n123, category = c("PPiSeq", "mDHFR-PCA", "BIOGRID data excluding mDHFR-PCA"), euler.d = TRUE,
                 scaled = TRUE, col = apple_colors[c(5,3,6)], fill = apple_colors[c(5,3,6)], cex = 1.5)
dev.off()
