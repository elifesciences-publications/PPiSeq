...
PPI_lineages = dataFrameReader_T("Paper_data/SD2_PPI_barcodes_fitness_counts.csv")
DMSO_mean = csvReader_T("Paper_data/SD2_mean_fitness_positive.csv") # 1459163
...
cor(as.numeric(PPI_fit_final[,2]), as.numeric(PPI_fit_final[,3]), method = "spearman") # 0.66
...
ggplot() +
        ...
        annotate("text", x = 0.3, y = 1.1, label = expression(paste("Spearman's ", italic(r), " = 0.66")),  parse = TRUE, col = apple_colors[11]) +
...
ggsave("~/Dropbox/PPiSeq_02/working_figure/SFigures/Figure1_related/FigureSX_Replicate_pos_all_environments_related_Figure1D/Figure1D_other_environments/Figure1D_correlation_two_replicates_hexagonlot_SD2.pdf", height =5, width =5)
