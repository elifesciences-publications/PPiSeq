...
for(i in 1:9){
        ...
        sum_row = rowSums(matrix_chosen, na.rm = T)
        vali_row = vali_rate_matrix[match(as.character(sum_row), vali_rate_matrix[,1]),2]
        environ_1_1 = sum(as.numeric(vali_row), na.rm = T)
...
for(h in 1:9){
        ...
        for(i in 1:8){
                ...
                sum_row = rowSums(matrix_chosen, na.rm = T)
                vali_row = vali_rate_matrix[match(as.character(sum_row), vali_rate_matrix[,1]),2]
                environ_2_1 = sum(as.numeric(vali_row), na.rm = T)
...
for(h in 1:9){
        ...
        for(i in 1:8){
                ...
                for(j in 1:7){
                        ...
                        sum_row = rowSums(matrix_chosen, na.rm = T)
                        vali_row = vali_rate_matrix[match(as.character(sum_row), vali_rate_matrix[,1]),2]
                        environ_3_1 = sum(as.numeric(vali_row), na.rm = T)
        ...
        for(i in 1:8){
                ...
                for(j in 1:7){
                        ...
                        for(k in 1:6){
                                ...
                                sum_row = rowSums(matrix_chosen, na.rm = T)
                                vali_row = vali_rate_matrix[match(as.character(sum_row), vali_rate_matrix[,1]),2]
                                environ_4_1 = sum(as.numeric(vali_row), na.rm = T)
                ...
                for(j in 1:7){
                        ...
                        for(k in 1:6){
                                ...
                                for(l in 1:5){
                                        ...
                                        sum_row = rowSums(matrix_chosen, na.rm = T)
                                        vali_row = vali_rate_matrix[match(as.character(sum_row), vali_rate_matrix[,1]),2]
                                        environ_5_1 = sum(as.numeric(vali_row), na.rm = T)
                        ...
                        for(k in 1:6){
                                ...
                                for(l in 1:5){
                                        ...
                                        for(m in 1:4){
                                                ...
                                                sum_row = rowSums(matrix_chosen, na.rm = T)
                                                vali_row = vali_rate_matrix[match(as.character(sum_row), vali_rate_matrix[,1]),2]
                                                environ_6_1 = sum(as.numeric(vali_row), na.rm = T)
                                ...
                                for(l in 1:5){
                                        ...
                                        for(m in 1:4){
                                                ...
                                                for(n in 1:3){
                                                        ...
                                                        sum_row = rowSums(matrix_chosen, na.rm = T)
                                                        vali_row = vali_rate_matrix[match(as.character(sum_row), vali_rate_matrix[,1]),2]
                                                        environ_7_1 = sum(as.numeric(vali_row), na.rm = T)
                                        ...
                                        for(m in 1:4){
                                                ...
                                                for(n in 1:3){
                                                        ...
                                                        for(o in 1:2){
                                                                ...
                                                                sum_row = rowSums(matrix_chosen, na.rm = T)
                                                                vali_row = vali_rate_matrix[match(as.character(sum_row), vali_rate_matrix[,1]),2]
                                                                environ_8_1 = sum(as.numeric(vali_row), na.rm = T)
...
matrix_count_1[,9] = sum(as.numeric(vali_row), na.rm = T)

colnames(matrix_count_1) = c("One", "Two", "Three", "Four", "Five", "Six", "Seven", "Eight", "Nine")

csvWriter(matrix_count_1, "Working_data_2/Accumulated_PPI_all/PPI_number_environment_accumulation_all_order_1_direct_calculate.csv")

##### Make a plot to show these accumulated counts
setwd("~/Dropbox/PPiSeq_02/Working_data_2/Accumulated_PPI_all/")
matrix_count_1 = dataFrameReader_T("PPI_number_environment_accumulation_all_order_1_direct_calculate.csv")
...
pdf("~/Dropbox/PPiSeq_02/Working_figure/Figure3_accessory_PPIs/Figure3D_PPI_number_accumulation_environments_validated_direct_calculate_no_model.pdf", width= 5.5, height =5.5)
#pdf("~/Desktop/Figure2B_PPI_number_accumulation_environments_combine.pdf", width =5, height =5)
par(mar = c(5,5,2,4))
plot(1:9, as.numeric(matrix_count_1[1,]), xlim = c(1,9), ylim = c(0,10000), type = "l",
     col = col_chosen[1],lwd = 0.5, axes=F,
     xlab = "Number of environments assayed",
     ylab = "Number of PPIs", main = NA, bty = "n")
axis(2, at= seq(0, 10000, by = 2000), labels = seq(0, 10000, by = 2000))
...
#lines(1:9, predict_PPI_plot, col = apple_colors[7], lwd = 2, lty = 2)
...
#### Fit a sigmoid model to the data and predict the maximum PPI number across environments
setwd("~/Dropbox/PPiSeq_02/Working_data_2/Accumulated_PPI_all/")
matrix_count_csv = csvReader_T("PPI_number_environment_accumulation_all_order_1_direct_calculate.csv")
PPI_number = as.numeric(as.vector(t(matrix_count_csv)))
env_num = rep(1:9, nrow(matrix_count_1))
fitmodel = nls(PPI_number  ~ SSlogis(env_num, Asym, xmid, scal))
coeff = coef(fitmodel)
env_num_plot = 1:9
predict_PPI_plot = coeff[1]/(1+ exp((coeff[2] - env_num_plot)/coeff[3]))
