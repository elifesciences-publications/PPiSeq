...
# Cellular compartment
cold_CC_density = as.matrix(read.table("16C/Changing_PPI_count_CC_density_matrix.txt", sep = "\t", header = T))
Dox_CC_density = as.matrix(read.table("Dox/Changing_PPI_count_CC_density_matrix.txt", sep = "\t", header = T))
FK506_CC_density = as.matrix(read.table("FK506/Changing_PPI_count_CC_density_matrix.txt", sep = "\t", header = T))
Forskolin_CC_density = as.matrix(read.table("Forskolin/Changing_PPI_count_CC_density_matrix.txt", sep = "\t", header = T))
H2O2_CC_density = as.matrix(read.table("H2O2/Changing_PPI_count_CC_density_matrix.txt", sep = "\t", header = T))
HU_CC_density = as.matrix(read.table("HU/Changing_PPI_count_CC_density_matrix.txt", sep = "\t", header = T))
NaCl_CC_density = as.matrix(read.table("NaCl/Changing_PPI_count_CC_density_matrix.txt", sep = "\t", header = T))
Raffinose_CC_density = as.matrix(read.table("Raffinose/Changing_PPI_count_CC_density_matrix.txt", sep = "\t", header = T))

cc_list = list(cold_CC_density,Dox_CC_density,FK506_CC_density, Forskolin_CC_density,
...
hist(mean)
max(as.numeric(network_density_vector), na.rm = T) # 0.01773948
...
ggplot() + 
  ...
  scale_color_gradient(low = apple_colors[10], high = apple_colors[5], 
                       breaks = c(0, 0.005, 0.01, 0.015, 0.018))+  
  scale_size(range = c(0,3), breaks = c(0, 0.005, 0.01, 0.015, 0.018)) +
...
ggsave("~/Dropbox/PPiseq_02/Working_figure/Figure2_PPI_enrichment_GO/all_PPI/heatmap_mean_density_changing_PPI_CC.pdf", width = 8, height =4.5)
...
cold_BP_density = as.matrix(read.table("16C/Changing_PPI_count_BP_density_matrix.txt", sep = "\t", header = T))
Dox_BP_density = as.matrix(read.table("Dox/Changing_PPI_count_BP_density_matrix.txt", sep = "\t", header = T))
FK506_BP_density = as.matrix(read.table("FK506/Changing_PPI_count_BP_density_matrix.txt", sep = "\t", header = T))
Forskolin_BP_density = as.matrix(read.table("Forskolin/Changing_PPI_count_BP_density_matrix.txt", sep = "\t", header = T))
H2O2_BP_density = as.matrix(read.table("H2O2/Changing_PPI_count_BP_density_matrix.txt", sep = "\t", header = T))
HU_BP_density = as.matrix(read.table("HU/Changing_PPI_count_BP_density_matrix.txt", sep = "\t", header = T))
NaCl_BP_density = as.matrix(read.table("NaCl/Changing_PPI_count_BP_density_matrix.txt", sep = "\t", header = T))
Raffinose_BP_density = as.matrix(read.table("Raffinose/Changing_PPI_count_BP_density_matrix.txt", sep = "\t", header = T))

BP_list = list(cold_BP_density,Dox_BP_density,FK506_BP_density, Forskolin_BP_density,
...
ggplot() + 
  ...
  scale_size(range = c(0,3), breaks = c(0, 0.005, 0.01, 0.015, 0.02, 0.025, 0.03)) +
  guides(color = guide_legend(), size = guide_legend())+
  scale_x_discrete(limits = rev(GO_order$x)) + 
...
ggsave("~/Dropbox/PPiseq_02/Working_figure/Figure2_PPI_enrichment_GO/all_PPI/heatmap_mean_density_changing_PPI_BP_chosen.pdf", width = 12, height =7.5)
...
ggplot() + 
  ...
  theme(axis.text.x = element_blank(), plot.margin = unit(c(1.5,0.2,0.2,2.2), "cm"),
        axis.text.y.left = element_text(size = 8, color = "black"), axis.title = element_blank())
ggsave("~/Dropbox/PPiseq_02/Working_figure/Figure2_PPI_enrichment_GO/all_PPI/heatmap_mean_density_changing_PPI_BP_all.pdf", width = 16, height =12)
...
setwd("~/Dropbox/PPiseq_02/Working_data_2/PPI_pair_GO/environment/")
matrix_CC_order = csvReader_T("Changing_PPI_ratio_single_CC_order.csv")
matrix_BP_order = csvReader_T("Changing_PPI_ratio_single_BP_order.csv")
## CC the same color  
...
pdf("~/Dropbox/PPiseq_02/Working_figure/Figure2_PPI_enrichment_GO/all_PPI/Changing_PPI_ratio_CC_order.pdf", width= 3, height=1.8)
par(mar = c(1,2,0.5,0.5))

barCenter = barplot(as.numeric(matrix_CC_order[,3]), horiz=F, beside=F, 
...
text(x= barCenter, y = -0.0002, labels = matrix_CC_order[,2], cex = 0.5, xpd = TRUE)
#legend(19,1.2, legend= c("Chromosome", "Nucleolus"), fill = col_chosen[c(2,3)], bty = "n", 
#border = FALSE, xpd = TRUE, cex = 0.6)
dev.off()

pdf("~/Dropbox/PPiseq_02/Working_figure/Figure2_PPI_enrichment_GO/all_PPI/Changing_PPI_ratio_BP_order.pdf", width= 4.8, height=1.8)
par(mar = c(1,2,0.5,0))

barCenter = barplot(as.numeric(matrix_BP_order[,3]), horiz=F, beside=F,
                    ylab="Coefficient variration",axisnames=F, border=NA, cex.lab = 0.5,
                    col = apple_colors[5], cex.axis = 0.5)
#text(x= barCenter, y = as.numeric(merge_ratio)*100, labels = counts_label, cex=0.8, xpd = TRUE)
text(x= barCenter, y = -0.0003, labels = matrix_BP_order[,2], srt = 60, cex = 0.4, xpd = TRUE)
