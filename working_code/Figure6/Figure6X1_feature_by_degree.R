...
gene_feature_matched= cbind(gene_feature_matched, GIcount[vScore_protein[,1]])
colnames(gene_feature_matched)[40] = "GIdegree"
features = c(colnames(gene_feature), "GIdegree")
...
feature_names = c("dN/dS", "Protein disorder", "Number of complexes" ,  "Protein length", "Phenotypic capacitance", 
                  ...
                  "Codon adaptation index",  "Expression level", "PPI degree, Biogrid", "Protein abundance", "GI degree" )
vScore = as.numeric(vScore_protein[,2])
degree = as.numeric(gene_feature_matched[,4])
degree_bins = matrix(c(0,5,6,10,11,20,21,30,31,40,41,50,51,1000, 0, 4, 5, 14, 15, 1000, 0 , 1000), 2, 11)
...
for(i in 1:ncol(f_cor)){
  ...
  barCenter = barplot(f_cor[o, i], horiz=T, beside=F, xlim=c(-0.6, 0.6), 
                      ...
                      axisnames=F, border=NA, cex.axis=0.7, col = apple_colors[2], main = paste("degree = ", colnames(f_cor)[i]))
...
text(-0.65, barCenter[2,], labels= rownames(fc), cex = 1, xpd = TRUE)
legend(.2, 5, c("0-4", "5-14", "15+"), bty = "n", title = "PPI degree", 
