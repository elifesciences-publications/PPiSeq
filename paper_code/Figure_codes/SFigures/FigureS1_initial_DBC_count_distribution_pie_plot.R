...
DMSO_lineage = csvReader_T("SFigure1_related_data/SD_all_lineage_trajectories.csv")
nrow(DMSO_lineage) # 6,855,881
barcodes_control = csvReader_T("PPiseq_PPI_barcodes/PPiseq_control_barcodes.csv")
barcodes_real = csvReader_T("PPiseq_PPI_barcodes/PPiseq_expected_barcodes_non_control.csv")
...
ggsave("Figures/SFigures/SFigure1/FigureS1A_barcode_counts_distribution_01.pdf", width = 5, height = 5)
...
ggplot() + aes(counts) +
        ...
        theme(axis.text.x = element_text(size = 12, color = "black"), 
              axis.text.y.left = element_text(size = 12, color = "black")) + 
        theme(text = element_text(size=14))
ggsave("Figures/SFigures/SFigure1/FigureS1A_barcode_counts_distribution_02.pdf", width = 6, height = 4)
...
ggplot(PPI_count_hist, aes(x = factor(1), y = Freq, fill= barcode_count)) +
        ...
        scale_fill_manual(values=apple_colors[c(3,4,5,6)],
                          ...
                          labels = c("1", "2", "3", "4"))
#guides(fill=FALSE)
ggsave("Figures/SFigures/SFigure1/FigureS1B_barcode_counts_per_PPI_real.pdf", width = 5, height = 5)
