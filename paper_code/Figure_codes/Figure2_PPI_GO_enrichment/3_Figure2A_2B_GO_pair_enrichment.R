...
all_count= as.matrix(read.table("Figure2_related_data_generated_by_python_script/Network_all_count_PPI_CC_new.txt",sep = "\t", header = T))
all_count_name = paste(all_count[,1], all_count[,2], sep = "_")
...
random_name_front = "Figure2_related_data_generated_by_python_script/random_network/random_network_density/random_network_CC_pos_count_"
...
GO_order = read.table("Figures/Figure2/GO_CC_order.txt",header = T, sep = "\t")
...
ggsave("Figures/Figure2/heatmap_density_CC_PPI_network.pdf", width = 8, height =4.5)
...
#### extract density from each random network and put them as a column into a matrix
all_count= as.matrix(read.table("Figure2_related_data_generated_by_python_script/Network_all_count_PPI_BP_new.txt",sep = "\t", header = T))
all_count_name = paste(all_count[,1], all_count[,2], sep = "_")
...
random_name_front = "Figure2_related_data_generated_by_python_script/random_network/random_network_density/random_network_BP_pos_count_"
...
GO_order = read.table("Figures/Figure2/GO_BP_order_primary.txt", header = T, sep = "\t")
...
ggsave("Figures/Figure2/heatmap_density_BP_PPI_network_primary.pdf", width = 16, height = 12)

## (2) Choose only 59 GO terms to make figures
network_density = as.matrix(read.table("Figure2_related_data_generated_by_python_script/Network_density_PPI_BP_matrix.txt", 
                                       sep = "\t", header = T))
colnames(network_density) = gsub("\\.", " ", colnames(network_density))
chosen_BP = as.matrix(read.table("Figure2_related_data_generated_by_python_script/chosen_BP_GO_59.txt", header = F, sep = "\t"))
...
GO_order = read.table("Figures/Figure2/GO_BP_order_chosen.txt",header = T, sep = "\t")
...
ggsave("Figures/Figure2/heatmap_density_BP_PPI_network_chosen.pdf",width = 12, height = 7.5)
