...
def sort_PPI_count(barcode_fitness, genotype_pos, barcode_pos, fitness_pos, distance_pos, G0_pos, output):
    ...
    for i in d:
        ...
        if genotype in genotype_dict:
            ...
            genotype_dict[genotype]['G3'].append(G3)
    ...
    for i in genotype_dict.keys():
        ...
        for j in range(kinds):
            ...
            G3.append(genotype_dict[i]['G3'][j])
    ...
    for PPI, count, barcode, fitness, distance, G0, G3, G6, G9, G12 in zip(PPI_name, counts, barcode, fitness, distance, G0, G3, G6, G9, G12):
        writer.writerow([PPI, count, barcode, fitness, distance, G0, G3, G6, G9, G12])
